...
class ScarlettForecast():
    ...
    def api(self):
        if self.command in scarlett_constants.FORECAST_CMDS.keys():
            logger.debug("** received %s, sending 'forecast command: %s'" %
                         (self.command,
                          scarlett_constants.FORECAST_CMDS[self.command]
                          ))
            logger.debug(self.f.hourly().data[0].temperature)
            fio_hourly = "%s degrees fahrenheit" % (
                self.f.hourly().data[0].temperature)
            ...
            by_day = self.f.daily()
            logger.debug("Daily Summary: %s" % (by_day.summary))
