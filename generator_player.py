...
class ScarlettPlayer(object):
	...
	def __init__(self, path):
		...
		# BOSSJONESTEMP # self.audioconvert = Gst.ElementFactory.make('audioconvert', None)
	...
	def _no_more_pads(self, element):
		...
		if not self._got_a_pad:
			logger.error("If we haven't gotten at least one decodable stream, raise an exception.")
		...
		if not self.finished:
			if message.type == Gst.MessageType.EOS:
				...
				if not self.got_caps:
					logger.error("If the stream ends before _notify_caps was called, this is an invalid file.")
